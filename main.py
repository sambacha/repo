...
if sys.version_info.major < 3:
    print(
        "repo: warning: Python 2 is no longer supported; "
...
global_options = optparse.OptionParser(
    usage="repo [-p|--paginate|--no-pager] COMMAND [ARGS]", add_help_option=False
)
...
global_options.add_option(
    "--time", dest="time", action="store_true", help="time repo command execution"
)
global_options.add_option(
    "--version",
    dest="show_version",
...
class _Repo(object):
    ...
    def _Run(self, name, gopts, argv):
        ...
        if (
            ...
            print("fatal: '%s' requires GITC to be available" % name, file=sys.stderr)
            return 1
        ...
        try:
        ...
        except (DownloadError, ManifestInvalidRevisionError, NoManifestException) as e:
            print(
                "error: in `%s`: %s" % (" ".join([name] + argv), str(e)),
                file=sys.stderr,
            )
            ...
            if gopts.time:
                if hours == 0:
                    ...
                    print(
                        "real\t%dh%dm%.3fs" % (hours, minutes, seconds), file=sys.stderr
                    )
...
def _Main(argv):
    ...
    opt.add_option(
        "--wrapper-path", dest="wrapper_path", help="location of the wrapper script"
    )
    ...
    try:
        try:
            ...
            if gopts.trace_python:
                ...
                tracer = trace.Trace(
                    count=False, trace=True, timing=True, ignoredirs=set(sys.path[1:])
                )
