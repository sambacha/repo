...
class ReviewableBranch(object):
    ...
    def GetPublishedRefs(self):
        ...
        output = self.project.bare_git.ls_remote(
            self.branch.remote.SshReviewUrl(self.project.UserEmail), "refs/changes/*"
        )
...
class Project(object):
    ...
    """
        ...
        if not (
            optimized_fetch
            and (ID_RE.match(self.revisionExpr) and self._CheckForImmutableRevision())
        ):
    ...
    """
        ...
        if branch.merge != self.revisionExpr:
            if branch.merge and self.revisionExpr:
                syncbuf.info(
                    self, "manifest switched %s...%s", branch.merge, self.revisionExpr
                )
            elif branch.merge:
    ...
    """
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                ...
                print(
                    ...
                    "then run `repo sync -l`." % (self.relpath,),
                    file=sys.stderr,
                )
                return False
            ...
            for f in files:
                ...
                try:
                    ...
                    if e.errno != errno.ENOENT:
                        ...
                        failed = True
            dirs[:] = [
                d for d in dirs if not os.path.lexists(os.path.join(root, d, ".git"))
            ]
            dirs_to_remove += [
                os.path.join(root, d)
                for d in dirs
        ...
        for d in reversed(dirs_to_remove):
            if platform_utils.islink(d):
                try:
                    ...
                    if e.errno != errno.ENOENT:
                        print(
                            "error: %s: Failed to remove: %s" % (d, e), file=sys.stderr
                        )
                        failed = True
        ...
        if failed:
            print(
                "error: %s: Failed to delete obsolete checkout." % (self.relpath,),
                file=sys.stderr,
            )
    ...
    """
        ...
        return (
            GitCommand(
                self, ["checkout", name, "--"], capture_stdout=True, capture_stderr=True
            ).Wait()
            == 0
        )
    ...
    """
        ...
        return (
            GitCommand(
                self, ["branch", "-D", name], capture_stdout=True, capture_stderr=True
            ).Wait()
            == 0
        )
    ...
    def GetDerivedSubprojects(self):
        ...
        for rev, path, url in self._GetSubmodules():
            ...
            relpath, worktree, gitdir, objdir = self.manifest.GetSubprojectPaths(
                self, name, path
            )
    ...
    def _RemoteFetch(
        ...
        if not self.manifest.IsMirror and is_sha1 and depth and git_require((1, 8, 3)):
        ...
        for try_n in range(retry_fetches):
            ...
            elif "error:" in gitcmd.stderr and "HTTP 429" in gitcmd.stderr:
                if not quiet:
                    print(
                        "429 received, sleeping: %s sec" % retry_cur_sleep,
                        file=sys.stderr,
                    )
            ...
            # If this is not last attempt, try 'git remote prune'.
                ...
                try_n < retry_fetches - 1
                and "error:" in gitcmd.stderr
                and "git remote prune" in gitcmd.stderr
                and not prune_tried
            ):
                prune_tried = True
                prunecmd = GitCommand(
                    self, ["remote", "prune", name], bare=True, ssh_proxy=ssh_proxy
                )
                ret = prunecmd.Wait()
                if ret:
                    break
    ...
    def _ApplyCloneBundle(self, initial=False, quiet=False, verbose=False):
        if initial and (
            self.manifest.manifestProject.config.GetString("repo.depth")
            or self.clone_depth
        ):
            return False
        ...
        try:
            ...
            if self.objdir != self.gitdir:
                ...
                if init_obj_dir or init_git_dir:
                    self._ReferenceGitDir(
                        self.objdir, self.gitdir, share_refs=False, copy_all=True
                    )
                try:
                    ...
                    if force_sync:
                        ...
                        try:
                            ...
                            return self._InitGitDir(
                                mirror_git=mirror_git, force_sync=False, quiet=quiet
                            )
                        except Exception:
                            raise e
                    raise e
            ...
            if init_git_dir:
                ...
                if ref_dir or mirror_git:
                    ...
                    repo_git = os.path.join(
                        ref_dir, ".repo", "projects", self.relpath + ".git"
                    )
                    worktrees_git = os.path.join(
                        ref_dir, ".repo", "worktrees", self.name + ".git"
                    )
                    ...
                    if ref_dir:
                        ...
                        _lwrite(
                            os.path.join(self.gitdir, "objects/info/alternates"),
    ...
    def _InitWorkTree(self, force_sync=False, submodules=False):
        ...
        if init_dotgit:
            ...
            self._ReferenceGitDir(
                self.gitdir, tmpdotgit, share_refs=True, copy_all=False
            )
        else:
            dotgit = realdotgit
    ...
    def getAddedAndRemovedLogs(
        ...
        logs["added"] = self._getLogs(
            selfId, toId, oneline=oneline, color=color, pretty_format=pretty_format
        )
        logs["removed"] = self._getLogs(
            toId, selfId, oneline=oneline, color=color, pretty_format=pretty_format
        )
        return logs
    ...
    class _GitGetByExec(object):
      ...
      """
        ...
        def rev_list(self, *args, **kw):
            ...
            if p.Wait() != 0:
                raise GitError(
                    "%s rev-list %s: %s" % (self._project.name, str(args), p.stderr)
                )
      ...
      """
            ...
            def runner(*args, **kwargs):
                ...
                if p.Wait() != 0:
                    raise GitError("%s %s: %s" % (self._project.name, name, p.stderr))
                r = p.stdout
                if r.endswith("\n") and r.index("\n") == len(r) - 1:
                    return r[:-1]
                return r
...
class _Failure(object):
    ...
    def Print(self, syncbuf):
        syncbuf.out.fail("error: %s/: %s", self.project.relpath, str(self.why))
        syncbuf.out.nl()
