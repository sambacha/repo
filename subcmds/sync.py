...
"""
    ...
    def _FetchHelper(self, opt, project, lock, fetched, pm, err_event, clone_filter):
    ...
    """
        ...
        try:
            ...
            self.event_log.AddSync(
                project, event_log.TASK_SYNC_NETWORK, start, finish, success
            )
    ...
    def _Fetch(self, projects, opt, err_event):
        ...
        pm = Progress(
            "Fetching projects", len(projects), always_print_percentage=opt.quiet
        )
        ...
        for project_list in objdir_project_map.values():
            ...
            if self.jobs > 1:
                t = _threading.Thread(target=self._FetchProjectList, kwargs=kwargs)
    ...
    """
        ...
        start = time.time()
        syncbuf = SyncBuffer(
            self.manifest.manifestProject.config, detach_head=opt.detach_head
        )
        success = False
        try:
            try:
    ...
    """
        ...
        for project in all_projects:
            ...
            if project.worktree:
                ...
                if syncjobs > 1:
                    t = _threading.Thread(target=self._CheckoutWorker, kwargs=kwargs)
                    # Ensure that Ctrl-C will not freeze the repo process.
                    t.daemon = True
                    threads.add(t)
                    t.start()
    ...
    def _GCProjects(self, projects, opt, err_event):
        ...
        for project in projects:
            ...
            if (
                not project.use_git_worktrees
                and len(project.manifest.GetProjectsWithName(project.name)) > 1
            ):
                print(
                ...
                if git_require((2, 7, 0)):
                    ...
                    print(
                        "%s: WARNING: shared projects are unreliable when using old "
    ...
    def Execute(self, opt, args):
        ...
        if self.gitc_manifest:
            ...
            args = [
                os.path.relpath(self.manifest.paths[path].worktree, os.getcwd())
...
def _VerifyTag(project):
    ...
    if not cur or re.compile(r"^.*-[0-9]{1,}-g[0-9a-f]{1,}$").match(cur):
        ...
        print(file=sys.stderr)
        print(
            "warning: project '%s' branch '%s' is not signed" % (project.name, rev),
            file=sys.stderr,
        )
        return False
