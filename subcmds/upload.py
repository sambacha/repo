...
"""
    ...
    def _SingleBranch(self, opt, branch, people):
        ...
        if answer is None:
            ...
            print(
                ...
                % (name, len(commit_list), len(commit_list) != 1 and "s" or "", date)
            )
    ...
    def _UploadAndReport(self, opt, todo, original_people):
            ...
            try:
                ...
                for label in labels:
                    if not re.match(r"^.+[+-][0-9]+$", label):
                        print(
                            'repo: error: invalid label syntax "%s": labels use forms '
                            "like CodeReview+1 or Verified-1" % (label,),
                            file=sys.stderr,
                        )
                        sys.exit(1)
                ...
                if destination:
                    ...
                    if (
                        ...
                        print(
                            "Please use `--destination %s` if this is intentional"
                            % destination
                        )
                        branch.uploaded = False
                        continue
        ...
        for branch in todo:
            if branch.uploaded:
                print(
                    "[OK    ] %-15s %s" % (branch.project.relpath + "/", branch.name),
                    file=sys.stderr,
                )
