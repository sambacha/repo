...
"""
    ...
    def _Options(self, p, gitc_init=False):
        ...
        g.add_option("--worktree", action="store_true", help=optparse.SUPPRESS_HELP)
        g.add_option(
            "--archive",
            dest="archive",
            action="store_true",
        ...
        g.add_option("--repo-rev", metavar="REV", help="repo branch or revision")
        g.add_option("--repo-branch", dest="repo_rev", help=optparse.SUPPRESS_HELP)
        g.add_option(
            "--no-repo-verify",
            dest="repo_verify",
            default=True,
    ...
    def _ShouldConfigureUser(self, opt):
        ...
        if not opt.quiet:
            ...
            print(
                ...
                % (mp.config.GetString("user.name"), mp.config.GetString("user.email"))
            )
            print(
    ...
    def _DisplayResult(self, opt):
        ...
        if not opt.quiet:
            print()
            print(
                "repo %shas been initialized in %s" % (init_type, self.manifest.topdir)
            )
