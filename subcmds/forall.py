...
"""
    ...
    def Execute(self, opt, args):
        ...
        try:
            ...
            print(
                "Got an error, terminating the pool: %s: %s" % (type(e).__name__, e),
                file=sys.stderr,
            )
            pool.terminate()
    ...
    def ProjectArgs(self, projects, mirror, opt, cmd, shell, config):
        for cnt, p in enumerate(projects):
            try:
                ...
                print(
                    "Project list error on project %s: %s: %s"
                    % (p.name, type(e).__name__, e),
                    file=sys.stderr,
                )
                return
            except KeyboardInterrupt:
                print("Project list interrupted", file=sys.stderr)
                return
...
def DoWork(project, mirror, opt, cmd, shell, cnt, config):
    ...
    p = subprocess.Popen(
        cmd, cwd=cwd, shell=shell, env=env, stdin=stdin, stdout=stdout, stderr=stderr
    )
