...
"""
    ...
    """
        ...
        if self._repo_hooks_project:
            ...
            e.setAttribute(
                "enabled-list", " ".join(self._repo_hooks_project.enabled_repo_hooks)
            )
            root.appendChild(e)
    ...
    def _Load(self):
        if not self._loaded:
            ...
            nodes = []
            nodes.append(
                self._ParseManifestXml(self.manifestFile, self.manifestProject.worktree)
            )
    ...
    def _ParseManifest(self, node_list):
        ...
        def recursively_add_projects(project):
            ...
            if project.relpath is None:
                raise ManifestParseError(
                    "missing path for %s in %s" % (project.name, self.manifestFile)
                )
            if project.relpath in self._paths:
                raise ManifestParseError(
                    "duplicate path %s in %s" % (project.relpath, self.manifestFile)
                )
        ...
        for node in itertools.chain(*node_list):
            ...
            if node.nodeName == "repo-hooks":
                ...
                if len(repo_hooks_projects) != 1:
                    raise ManifestParseError(
                        "internal error parsing repo-hooks in %s" % (self.manifestFile)
                    )
    ...
    """
        ...
        if parent is None:
            relpath, worktree, gitdir, objdir, use_git_worktrees = self.GetProjectPaths(
                name, path
            )
        else:
    ...
    def _CheckLocalPath(path, symlink=False):
        ...
        if (
            norm == ".."
            or (len(norm) >= 3 and norm.startswith("..") and resep.match(norm[0]))
